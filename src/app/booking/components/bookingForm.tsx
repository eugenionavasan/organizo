...
export const BookingForm: React.FC<BookingFormProps> = ({ date, time, onBooking, onTimeBooked }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentOption === 'now') {
      paymentSucceeded = await processPayment(stripe, elements, price, setPaymentError);
    } else {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <FormInput label="Phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} required />
        ...
        <FormSelect
          ...
          options={Object.entries(SERVICE_PRICES).map(([key, value]) => ({ value: key, label: key }))}
          required
        />
        
        <PaymentSection paymentOption={paymentOption} setPaymentOption={setPaymentOption} />
